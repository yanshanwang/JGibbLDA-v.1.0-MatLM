...
public class Inferencer {	
	...
	public Model inference( LDADataset newData) throws IOException{
		...
		for (newModel.liter = 1; newModel.liter <= niters; newModel.liter++){
			...
			for (int m = 0; m < newModel.M; ++m){
				for (int n = 0; n < newModel.data.docs[m].length; n++){
					...
					newModel.z[m].set(n, topic); //z[m], the topic assignments for each word in doc m.
				}
			}//end foreach new doc
	...
	public Model inference(String [] strs) throws IOException{
	...
	public Model inference() throws IOException{	
		...
		computeNewPhi();
		newModel.liter--;
		newModel.saveModel(newModel.dfile + "." + newModel.modelName,option);		
