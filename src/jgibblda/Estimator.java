...
public class Estimator {
	...
	public boolean init(LDACmdOption option) throws IOException{
		this.option = option;
		File modelDirFile = new File(option.modelDir);
		...
		if (option.est){
			if (!trnModel.initNewModel(option))
				return false;
			trnModel.data.localDict.writeWordMap(option.modelDir + File.separator + option.wordMapFileName);
		}
		else if (option.estc){
	...
	public void estimate(){
		...
		for (trnModel.liter = lastIter + 1; trnModel.liter < trnModel.niters + lastIter; trnModel.liter++){
			...
			if (option.savestep > 0){
				if (trnModel.liter % option.savestep == 0){
					...
					computeTheta();
					computePhi();
					trnModel.saveModel(trnModel.modelName + Conversion.ZeroPad(trnModel.liter, 5),option);
				}
			}
		}// end iterations		
		...
		computePhi();
		trnModel.liter--;
		trnModel.saveModel(trnModel.modelName+"-final",option);
	}
